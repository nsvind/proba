...
class DB {
    ...
    private $dbname = "datain_procena_nepokretnosti";
    private $dbusername = "datain_v2dbusr";
    private $dbpassword = "wkRZ9bd8";
    ...
    function SelectFromComments($id){
        try {
            ...
            $sql = "SELECT id_realestate, comment_admin, price FROM comments";
            
            if ($id) {
                $sql .= " WHERE id_realestate = ?";
            } else {
                $sql .= " WHERE id_admin is not null";
            }
    ...
    function SelectFromRealestateLimit($limit = 20, $checked) {
        ...
        try {
            $sql = "SELECT r.id_realestate, r.email_client, r.date, m.name AS m_name, c.community 
                        FROM realestate AS r
                        left join municipality AS m on m.id_municipality = r.id_municipality
    ...
    function CountFromRealestateCheckbox($checked, $partialName = '') {
        ...
        try {
            $sql = "SELECT count(*) as broj
                       ...
                       ";
            if ($checked) {
                $sql .= " where r.id_realestate is not null"; // promenuo co.id u r.id
            } else {
...
//                }
    ...
    function SelectFromRealestateCheckbox($limit = 20, $offset= 0, $checked, $partialName = '') {
        ...
        try {
            $sql = "SELECT r.id_realestate, r.email_client, r.address_client, r.date, m.name AS m_name, c.community 
                        FROM realestate AS r
                        left join municipality AS m on m.id_municipality = r.id_municipality
                        left join catastral_municipality AS c on c.id_catastral = r.id_catastral      
                        left join comments AS co on r.id_realestate = co.id_realestate
